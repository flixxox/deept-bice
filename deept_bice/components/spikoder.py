...
def create_spikoder(
    ...
    elif spikder_descr == 'population':
        return PopulationSpikoder(
            input_dim, output_dim, encoding_length,
            sample_labels, resample_every_step
        )
    else:
...
class PopulationSpikoder(nn.Module):
    ...
    def compute_all_labels(self):
        ...
        neurons_per_label = J // C

        labels = []
        for c in range(C):

            zeros_before = neurons_per_label * c
            zeros_after = J-(neurons_per_label*(c+1))
            
            c_seq = torch.cat(
                [
                    torch.zeros(T_l, zeros_before),
                    torch.ones(T_l, neurons_per_label),
                    torch.zeros(T_l, zeros_after)
                ], dim=1
            ).unsqueeze(0)
